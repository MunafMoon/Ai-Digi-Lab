...
type View = "Board" | "Backlog" | "Sprints" | "Roadmap" | "Calendar" | "AI Assistant" | "Documents" | "Reports" | "Notifications" | "Admin";
...
const tabs: View[] = ["Board", "Backlog", "Sprints", "Roadmap", "Calendar", "AI Assistant", "Documents", "Reports", "Notifications", "Admin"];
...
function DocumentsView() {
  return <section className="grid gap-3 md:grid-cols-2"><article className="rounded-md border border-slate-200 bg-white p-4"><FileText className="text-iris" size={20} /><h3 className="mt-3 font-semibold">E-Commerce Platform Requirements</h3><p className="mt-2 text-sm leading-6 text-slate-700">Authentication must support email login and OAuth. Checkout flows should handle timeouts, retries, and refund policy notes.</p><button className="mt-4 rounded-md border border-slate-200 px-3 py-2 text-sm">Ask AI about this doc</button></article><article className="rounded-md border border-slate-200 bg-white p-4"><FileText className="text-mint" size={20} /><h3 className="mt-3 font-semibold">Architecture Notes</h3><p className="mt-2 text-sm leading-6 text-slate-700">Project data remains deterministic. AI can cite documents and propose changes, but approvals create the final source-of-truth records.</p><button className="mt-4 rounded-md border border-slate-200 px-3 py-2 text-sm">Open document</button></article></section>;
}
...
function ReportsView({ tasks }: { tasks: Task[] }) {
  ...
  return <section className="grid gap-3 md:grid-cols-3"><article className="rounded-md border border-slate-200 bg-white p-4"><h3 className="font-semibold">Completion</h3><p className="mt-3 text-2xl font-semibold">{Math.round((completed.length / Math.max(tasks.length, 1)) * 100)}%</p><p className="mt-2 text-sm text-slate-600">{completed.length} of {tasks.length} tasks done.</p></article><article className="rounded-md border border-slate-200 bg-white p-4"><h3 className="font-semibold">Story points</h3><p className="mt-3 text-2xl font-semibold">{completedPoints}/{totalPoints}</p><p className="mt-2 text-sm text-slate-600">Completed versus total planned points.</p></article><article className="rounded-md border border-slate-200 bg-white p-4"><h3 className="font-semibold">Bug trend</h3><p className="mt-3 text-2xl font-semibold">1 open</p><p className="mt-2 text-sm text-slate-600">Checkout timeout remains in QA.</p></article></section>;
}

function NotificationsView() {
  const notifications = ["ECOM delivery risk detected", "You were mentioned on ECOM-1", "Sprint Alpha started", "ECOM-2 is due soon"];
  return <section className="rounded-md border border-slate-200 bg-white p-4"><h3 className="font-semibold">Notifications</h3><div className="mt-3 space-y-2 text-sm text-slate-700">{notifications.map((notification) => <div key={notification} className="flex items-center justify-between rounded-md bg-slate-50 p-3"><span><Bell className="mr-2 inline text-iris" size={15} />{notification}</span><button className="rounded border border-slate-200 bg-white px-2 py-1 text-xs">Read</button></div>)}</div></section>;
}


function AdminView() {
  const plans = [
    { name: "Free", usage: "50 AI requests", controls: "Basic project workspace" },
    { name: "Pro", usage: "500 AI requests", controls: "Reports and document AI" },
    { name: "Business", usage: "2,500 AI requests", controls: "Admin, audit, security controls" }
  ];
  const usage = [{ label: "AI requests today", value: "38 / 100" }, { label: "Monthly AI spend", value: "$18 / $100" }, { label: "Audit events", value: "142" }];
  const checks = ["Tenant isolation checks", "Role based API guards", "AI budget controls", "Audit logging", "Upload validation", "Environment secret checks"];
  const audit = ["AI settings updated by Alex", "Business plan reviewed", "Document upload validated", "Security checklist opened"];

  return <section className="space-y-4"><div className="grid gap-3 md:grid-cols-3">{usage.map((item) => <article key={item.label} className="rounded-md border border-slate-200 bg-white p-4"><p className="text-sm text-slate-500">{item.label}</p><p className="mt-3 text-2xl font-semibold">{item.value}</p></article>)}</div><div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]"><div className="rounded-md border border-slate-200 bg-white p-4"><h3 className="font-semibold">Billing plans</h3><div className="mt-3 grid gap-3 md:grid-cols-3">{plans.map((plan) => <article key={plan.name} className="rounded-md border border-slate-200 p-3"><p className="font-semibold">{plan.name}</p><p className="mt-2 text-sm text-slate-600">{plan.usage}</p><p className="mt-2 text-sm text-slate-600">{plan.controls}</p><button className="mt-4 w-full rounded-md border border-slate-200 px-3 py-2 text-sm">Manage</button></article>)}</div></div><div className="rounded-md border border-slate-200 bg-white p-4"><h3 className="font-semibold">AI controls</h3><div className="mt-3 space-y-3 text-sm"><label className="block"><span className="text-slate-600">Daily request limit</span><input className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2" value="100" readOnly /></label><label className="block"><span className="text-slate-600">Monthly budget</span><input className="mt-1 w-full rounded-md border border-slate-200 px-3 py-2" value="$100" readOnly /></label><button className="w-full rounded-md bg-ink px-3 py-2 font-medium text-white"><Settings className="mr-1 inline" size={16} /> Save controls</button></div></div></div><div className="grid gap-4 lg:grid-cols-2"><div className="rounded-md border border-slate-200 bg-white p-4"><h3 className="font-semibold">Security checklist</h3><div className="mt-3 space-y-2 text-sm text-slate-700">{checks.map((check) => <p key={check} className="rounded bg-slate-50 p-2"><CheckCircle2 className="mr-2 inline text-mint" size={15} />{check}</p>)}</div></div><div className="rounded-md border border-slate-200 bg-white p-4"><h3 className="font-semibold">Audit trail</h3><div className="mt-3 space-y-2 text-sm text-slate-700">{audit.map((event) => <p key={event} className="rounded bg-slate-50 p-2"><Bell className="mr-2 inline text-iris" size={15} />{event}</p>)}</div></div></div></section>;
}
